...
<template>
  <checkbox-group ref="hlCheckBoxRef" v-model="checkBoxValue" :direction="direction" :icon-size="iconSize" shape="square" v-bind="$attrs" @change="handleChange">
    <checkbox v-for="item in options" :key="item.value" :name="item.value" class="m-2">
      {{ item.label }}
    </checkbox>
  </checkbox-group>
</template>
